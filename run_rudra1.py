...
def run_test(test_case:TestCase):
    Rudra1.run_rudra_cmd(test_case)
...
if __name__ == "__main__":
    ...
    rudra1_report_dir = [d.replace(CVE_REPO_DIR,RUDRA1_REPORT_DIR) for d in cve_dirs]
    for d in rudra1_report_dir:
        if not os.path.exists(d):
            os.makedirs(d)
    test_cases = filter(
        lambda t: t is not None,
        map(lambda path,report_path: TestCase.create_test_case(path,report_path), cve_repos,rudra1_report_dir)
    )

    progress_bar = tqdm(total=len(rudra1_report_dir), desc="Processing test cases")
