...
if __name__ == "__main__":
    ...
    mirai_report_dir = [os.path.dirname(os.path.dirname(d.replace(CVE_REPO_DIR,MIRAI_REPORT_DIR))) for d in cve_repos]

    for d in mirai_report_dir:
        if not os.path.exists(d):
            os.makedirs(d)
    test_cases = list(filter(
        lambda t: t is not None,
        map(lambda path,report_path: TestCase.create_test_case(path,report_path), cve_repos,mirai_report_dir)
    ))
