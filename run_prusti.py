...
def run_test(test_case:TestCase):
    ...
    # env_dict["PRUSTI_ALLOW_UNREACHABLE_UNSUPPORTED_CODE"] = "true"

    
    
    Prusti.run_prusti_cmd(test_case,env_dict)
    return test_case
...
if __name__ == "__main__":
    ...
    for index, row in df.iterrows():
        ...
        prusti_report_path = os.path.dirname(os.path.dirname(cve_repo_path.replace(CVE_REPO_DIR,PRUSTI_REPORT_DIR)))
        if not os.path.exists(prusti_report_path):
            os.makedirs(prusti_report_path)
        else:
            shutil.rmtree(prusti_report_path)
            os.makedirs(prusti_report_path)
    ...
    output_file = "prusti_reports_state.csv"
