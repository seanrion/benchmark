...
def run_test(test_case:TestCase):
    Semgrep.run_semgrep_cmd(test_case)
    return test_case
...
if __name__ == "__main__":
    ...
    #     map(lambda path: path if ("GHSA-2f5j-3mhq-xv58, RUSTSEC-2020-0100" in path) else None, cve_dirs)))
    ...
    semgrep_report_dir = [os.path.dirname(os.path.dirname(d.replace(CVE_REPO_DIR,SEMGREP_REPORT_DIR))) for d in cve_repos]

    for d in semgrep_report_dir:
        if not os.path.exists(d):
            os.makedirs(d)
    test_cases = list(filter(
        lambda t: t is not None,
        map(lambda path,report_path: TestCase.create_test_case(path,report_path), cve_repos, semgrep_report_dir)
    ...
    with open("./semgrep.result",'w') as f:
